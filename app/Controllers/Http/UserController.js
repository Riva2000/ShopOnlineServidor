...
async deleteDepartamento({params, response})
{
...
{
  const paq=await Paqueteria.find(params.id)
  if(!paq)
  {
    return response.status(404).json({data: "Datos no encontrados"})
  }
...
{
  const pro=await Producto.find(params.id)
  if(!pro)
  {
    return response.status(404).json({data: "Datos no encontrados"})
  }
...
async deleteDetalle_venta({params, response})
{
  const dev=await DetalleVenta.find(params.id)
  if(!dev)
  {
    return response.status(404).json({data: "Datos no encontrados"})
  }
...
{
  const sup=await Proveedore.find(params.id)
  if(!sup)
  {
    return response.status(404).json({data: "Datos no encontrados"})
  }
